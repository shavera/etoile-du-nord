...
class ElementsGeneratorTest : public TestWithParam<std::string> {
 public:
  ...
  const OrbitalKernel unitCircleParams{
      StandardGravParam{1},
      StateVector{PositionVector{{1}, {}, {}}, VelocityVector{{}, {1}, {}}},
      {}};
  ...
  const OrbitalKernel unitFreeFallParams{
      StandardGravParam{1},
      StateVector{PositionVector{{1}, {}, {}}, VelocityVector{{}, {}, {}}},
      {}};
  ...
  const TestExpectations freefallExpectations{
      ...
      {Angle::radians(kPi)},  // radial orbits are weird case here - will need a
                              // few extra tests
      {std::nan("not sure on radial orbits")},  // not sure here if this can be
                                                // defined
      {std::nan(
          "not sure on radial orbits")}  // not sure if this can be defined
  };
  ...
  const OrbitalKernel parabolicOrbitParams{
      ...
      StateVector{PositionVector{{6}, {-18}, {1}},
                  VelocityVector{{14}, {}, {}}},
      {}};
  // ang mom = (0, 14, -252) = 70*sqrt(13); h^2 = 63700; norm = (0,
  // 1/(5sqrt(13), 18/(5sqrt(13)) rp = h^2 / 2*mu = 17.1052631579;
  // M=sqrt(mu/(2*rp^3))=0.4313007372 incl = acos(18/5sqrt(13)) = 3.1798301199
  // degrees ascNodeVec = {-14, 0, 0} -> Norm = {-1, 0, 0} longAscNode =
  // atan2(hx, -hy) = atan2(0, 14) = pi Vxh = {0, -14*-252, 14*14} = {0, -3528,
  // 196} ; Vxh/mu = {0, -36/19, 2/19} r_hat = {6/19, -18/19, 1/19} eccVec =
  // {-6/19, (-36+18)/19, (2-1)/19} = {-6/19, -18/19, 1/19} eccVecNorm -> same
  // as {-6, -18, 1}, which has norm 19, therefore is already normalized (should
  // be, parabolic case ecc=1) argPeri -> acos(6/19) = 71.5915198294° distance
  // periapsis -> rp see above semi-latus rectum = 2*rp = h^2/mu = 63700/1862
  // = 34.2105263158
  ...
  const TestExpectations parabolicExpectations{
      ...
      {0.4313007372},  // subtly different meaning of "sweep" for parabolic
                       // orbits
  ...
  const OrbitalKernel hyperbolicOrbitParams{
      StandardGravParam{1862},
      StateVector{PositionVector{{18}, {6}, {1}},
                  VelocityVector{{-10}, {11}, {-2}}},
      {}};
  const ElementsGenerator hyperbolicOrbit{hyperbolicOrbitParams};
  // ang mom = {-23, 26, 258} = 13*sqrt(401); norm = (-23/13sqrt(401),
  // 26/13sqrt(401), 258/13sqrt(401)) incl = acos(258/13sqrt(401))
  // = 7.6629523069 degrees ascNodeVec = {-26, -23, 0} -> Norm =
  // {-26/sqrt(1205), -23/sqrt(1205), 0} longAscNode = atan2(-23, -26) =
  // -2.417342652841646 rad Vxh = {(11*258)-(-2*26), (-2*-23)-(-10*258),
  // (-10*26)-(11*-23)} = {2890, 2626, -7} r_hat = {18/19, 6/19, 1/19} =
  // {1764/1862, 588/1862, 98/1862} eccVec = {(2890-1764)/1862, (2626-588)/1862,
  // (-7-98)/1862} = {1126/1862, 2038/1862, -105/1862} (norm =
  // sqrt(5432345)/1862 = 1.25054... (ecc) eccVecNorm same as {1126, 2038, -105}
  // -> (1/sqrt(5432345))*{2548, 2518, -105} argPeri = acos((-26*1126 + -23*2038
  // )/(sqrt(1205)*sqrt(5432345)) = acos(-76150/(sqrt(1205*5432345)) =
  // 160.2543589611° p (s-l r), q (d peri) are same as in elliptical case p =
  // a(1-e^2), q=a(1-e), where a<0
  const double hyperbolicEcc =
      hyperbolicOrbitParams.eccentricityVector().norm();
  ...
  // ang mom = {58, 6, -240} = 10*sqrt(610); norm = (29/5sqrt(610),
  // 3/5sqrt(610), -120/5sqrt(610))) incl = acos(-12*sqrt(2/305))=166.3442145528
  // degrees - corresponds to left hand orbit choice ascNodeVec = {-6, 58, 0} ->
  // Norm = {-6/(10*sqrt(34)), 58/(10*sqrt(34)), 0} = {-3/5sqrt(34),
  // 29/5sqrt(34, 0} longAscNode = atan2(58, -6) = 1.6738779353175968 Vxh =
  // {(4*-240)-(3*6), (3*58)-(12*-240), (12*6)-(4*58)} = {-978, 3054, -160}
  // r_hat = {-6/19, 18/19, -1/19} = {-588/1862, 1764/1862, -98/1862}
  // eccVec = {(-978+588)/1862, (3054-1764)/1862, (-160+98)/1862} =
  // (1/1862)*{-390, 1290, -62} = (1/931){-195, 645, -31} - ecc = 0.7245381653
  // eccVecNorm same as {-195, 645, -31} -> (1/sqrt(455011))*{-195, 645, -31}
  // argPeri = acos((-195*-3+645*29)/(5*sqrt(34)*sqrt(455011)) =
  // acos(19290/(5*sqrt(34*455011)) = acos(3858/sqrt(15470374)) = 11.2248489655°
  const double ellipticalEcc =
      ellipticalOrbitParams.eccentricityVector().norm();
...
TEST_P(ElementsGeneratorTest, semiMajorAxis) {
  const ElementsGenerator& orbit = testCaseMap.at(GetParam());
  const Meters& expectedSemiMajorAxis =
      testExpectationsMap.at(GetParam()).semiMajorAxis;
...
TEST_P(ElementsGeneratorTest, eccentricity) {
  ...
  const double expectedEccentricity =
      testExpectationsMap.at(caseName).eccentricity;
...
TEST_P(ElementsGeneratorTest, sweep) {
  const ElementsGenerator& orbit = testCaseMap.at(GetParam());
  const RadiansPerSecond expectedSweep =
      testExpectationsMap.at(GetParam()).sweep;
...
TEST_P(ElementsGeneratorTest, inclination) {
  const ElementsGenerator& orbit = testCaseMap.at(GetParam());
  const Angle expectedInclination =
      testExpectationsMap.at(GetParam()).inclination;
  ...
  // note: this comparison will do very poorly around ± pi radians, but probably
  // not a problem in this test.
  EXPECT_NEAR(expectedInclination.getDegrees(), actualInclination.getDegrees(),
              1e-9);
}

TEST_P(ElementsGeneratorTest, longitudeOfAscendingNode) {
  const ElementsGenerator& orbit = testCaseMap.at(GetParam());
  const Angle expectedLongAscNode =
      testExpectationsMap.at(GetParam()).longitudeOfAscendingNode;
...
TEST_P(ElementsGeneratorTest, argumentOfPeriapsis) {
  const ElementsGenerator& orbit = testCaseMap.at(GetParam());
  const Angle expectedArgPeriapsis =
      testExpectationsMap.at(GetParam()).argumentOfPeriapsis;
...
TEST_P(ElementsGeneratorTest, periapsisDistance) {
  if ("freeFall" == GetParam()) {
    SUCCEED() << "this isn't meaningful for radial trajectories, need to work "
                 "on that.";
    return;
  }

  const ElementsGenerator& orbit = testCaseMap.at(GetParam());
  const Meters expectedDistance =
      testExpectationsMap.at(GetParam()).periapsisDistance;
