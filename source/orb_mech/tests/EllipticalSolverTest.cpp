...
TEST(EllipticalSolverCtorTest, constructorBootstrap){
  // can just use a simple trivial case here - circular, initial position unit-x
...
TEST(EllipticalSolverCtorTest, throwsOnZeroPosition){
  const CartesianVector eccentricityVector{0,0,0};
  const PositionVector initialPosition{{0}, {},{}};
  const Seconds initialEpochTime{1.234};
  EXPECT_THROW((EllipticalSolver{eccentricityVector, initialPosition, initialEpochTime}), std::runtime_error);
}
...
TEST(EllipticalSolverUpdateTest, throwsOnZeroPosition){
  const CartesianVector eccentricityVector{0,0,0};
  const PositionVector initialPosition{{1}, {},{}};
  const Seconds initialEpochTime{1.234};
  EllipticalSolver solver{eccentricityVector, initialPosition, initialEpochTime};
  EXPECT_THROW((solver.updateStateAtEpoch(eccentricityVector, {{},{},{}}, {3.45})), std::runtime_error);
}

enum class CaseShape{circular, elliptical};
...
using TestDatum = std::tuple<CaseShape, CaseOrientation, AnomalySign, AnomalyBase>;
...
class EllipticalSolverTest : public TestWithParam<TestDatum>{
public:
  void SetUp() override{
    ...
    const auto& [shape, orientation, sign, base] = GetParam();

    const CartesianVector& eccVec = CaseShape::circular == shape ? circEccVec : ellpEccVec;
    ...
    expectedMeanAnomaly = CaseShape::circular == shape ? trueAnomaly : getExpectedMeanAnomaly(base, sign);
  }

  const CartesianVector circEccVec{0,0,0};
  const CartesianVector ellpEccVec{0.25, 0, 0}; // circular should, by default, treat the x axis as the "eccentric" axis to measure from - reusing here makes stuff easier
  const PositionVector initialPosition{{1}, {0},{0}};
  const Seconds initialEpochTime{1};
  EllipticalSolver solver{circEccVec, initialPosition, initialEpochTime};

  PositionVector testPosition{initialPosition};
  CartesianVector testEccVec{circEccVec};
...
std::string printer(const TestParamInfo<EllipticalSolverTest::ParamType>& info){
    const auto& [shape, orientation, sign, base] = info.param;
    std::string out{};
    out += CaseShape::circular == shape ? "Circ" : "Ellp";
...
INSTANTIATE_TEST_SUITE_P(
    ...
    Combine(Values(CaseShape::circular, CaseShape::elliptical),
            Values(CaseOrientation::planar, CaseOrientation::rotated),
