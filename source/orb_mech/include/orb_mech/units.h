...
class Angle {
 public:
  ...
  static Angle degrees(double deg) { return Angle{Angle::deg_to_rad(deg)}; }
  static Angle radians(double rad) { return Angle{rad}; }

  static Angle Zero() { return Angle{0.0}; }

  [[nodiscard]] double getDegrees() const {
    return rad_ * 180.0 / std::numbers::pi;
  }
  ...
  bool operator==(const Angle& other) const { return rad_ == other.rad_; }
 ...
 private:
  ...
  explicit Angle(double rad) : rad_{normalizeRadians(rad)} {}
